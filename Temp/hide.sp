...
bool g_bHide[MAXPLAYERS+1] =  { true, ... };
...
public void OnPluginStart()
{
	...
	g_cOnlyTeammates = CreateConVar("sm_hideplayers_only_teammates", "0", "Hide only teammates of client? | 1 = Yes | 0 = No");
		...
		if (g_cEnabled.BoolValue) {
			if (g_cOnlyForAdmins.BoolValue) {
				...
				if (bAdmin != INVALID_ADMIN_ID) {
					...
					PrintToChat(client, "%s", g_bHide[client] ? "You're hided the players.":"You will see other players after this!");
                    return Plugin_Handled;
				}

				else {
					PrintToChat(client, "You must be admin to use this command.");
                    return Plugin_Handled;
				}
			}
				...
				g_bHide[client] = !g_bHide[client];
				PrintToChat(client, "%s", g_bHide[client] ? "You're hided other players.":"You will see other players after this!");
                return Plugin_Handled;
			}
		}

		else {
			PrintToChat(client, "Plugin disabled.");
            return Plugin_Handled;
		}
	}
