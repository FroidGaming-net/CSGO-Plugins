...
public void OnPluginStart()
{
	...
	Cvar_Vol = CreateConVar("mvp_defaultvol", "1.0", "Default MVP anthem volume.", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	if (IsValidClient(client) && !IsFakeClient(client))
	{
		for(int i = 1; i <= MaxClients; i++)
		{
			if (IsValidClient(i) && !IsFakeClient(i))
			{
				...
				PrintHintText(i, "\n\t<font class='fontSize-xl'>Playing <font color='#FF0000'>%s</font></font>", g_sMVPName[mvp]);
