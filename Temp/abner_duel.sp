...
public void OnPluginStart()
{
	...
	g_hSoundPath 										= CreateConVar("duel_music_path", "froidgaming/misc/fight.mp3", "Duel Sounds Path");
	...
	/*                                                                      ClientPrefs	    																				*/
	g_DuelCookie 									 	= RegClientCookie("Duel Settings", "", CookieAccess_Private);
	g_SoundsCookie 										= RegClientCookie("abner_duel_sounds", "", CookieAccess_Private);

	SetCookieMenuItem(DuelCookieHandler, 0, "Duel");
...
public Action DuelMenu(int client, int args)
{
	...
	SetMenuTitle(g_DuelMenu, "Duel Settings");
	switch(cookievalue)
	{
		case 0:
		{
