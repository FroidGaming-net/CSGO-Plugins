...
public void OnPluginStart()
{
	...
	CSetPrefix("{default}[{lightblue}WeaponStickers{default}]");
...
void SetWeaponSticker(int client, int entity)
{
	if (IsClientInGame(client) && !IsFakeClient(client) && IsValidEntity(entity))
	{
		...
		if (IsValidDefIndex(iDefIndex) && ClientWeaponHasStickers(client, iDefIndex))
		{
			int index = eItems_GetWeaponNumByDefIndex(iDefIndex);
			if (index != -1)
			{
				...
				for (int i = 0; i < MAX_STICKERS_SLOT; i++)
				{
					if (g_PlayerWeapon[client][index].m_sticker[i] != 0)
					{
						// Sticker updated.
						isUpdated = true;
						pAttributeList.SetOrAddAttributeValue(113 + i * 4, g_PlayerWeapon[client][index].m_sticker[i]); //sticker slot %i id
					}
				}
