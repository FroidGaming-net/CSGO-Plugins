...
{
	g_cvAnnounceMainWebhook = CreateConVar("sm_surftimer_discord_announce_main_webhook", "https://discord.com/api/webhooks/867223541005811742/rEbGf-f2VWjZyrhoSeVC4zN-k2tIII8YhbQwBpFXvCbj9-F9O2txI3jghMvCkN6uhWTW", "The webhook to the discord channel where you want main record messages to be sent.", FCVAR_PROTECTED);
	g_cvAnnounceBonusWebhook = CreateConVar("sm_surftimer_discord_announce_bonus_webhook", "https://discord.com/api/webhooks/867223541005811742/rEbGf-f2VWjZyrhoSeVC4zN-k2tIII8YhbQwBpFXvCbj9-F9O2txI3jghMvCkN6uhWTW", "The webhook to the discord channel where you want bonus record messages to be sent.", FCVAR_PROTECTED);
	g_cvReportBugsDiscord = CreateConVar("sm_surftimer_discord_report_bug_webhook", "https://discord.com/api/webhooks/867636691057508363/0TQ-dBArovndIhhZ4rN6BTIAZ0-bj8RfRpb9fh0BYSK8gveZYq1hFUnO2NG1kw3Ri4_K", "The webhook to the discord channel where you want bug report messages to be sent.", FCVAR_PROTECTED);
	g_cvCallAdminDiscord = CreateConVar("sm_surftimer_discord_calladmin_webhook", "https://discord.com/api/webhooks/867636691057508363/0TQ-dBArovndIhhZ4rN6BTIAZ0-bj8RfRpb9fh0BYSK8gveZYq1hFUnO2NG1kw3Ri4_K", "The webhook to the discord channel where you want calladmin messages to be sent.", FCVAR_PROTECTED);
	g_cvAnnounceMention = CreateConVar("sm_surftimer_discord_announce_mention", "", "Optional discord mention to ping users when a new record has been set.");
	...
	g_cvBotUsername = CreateConVar("sm_surftimer_discord_username", "SurfTimer", "Username of the bot");
	...
	g_cvSteamWebAPIKey = CreateConVar("sm_surftimer_discord_steam_api_key", "88E56F32817CD873DBADC36F9BD413ED", "Allows the use of the player profile picture, leave blank to disable. The key can be obtained here: https://steamcommunity.com/dev/apikey", FCVAR_PROTECTED);
...
{
	CPrintToChat(client, "{blue}[SurfTimer-Discord] {green}Type your message. Type 'cancel' to abort.");
...
public int ReportBugHandler(Menu menu, MenuAction action, int param1, int param2)
{
	if(action == MenuAction_Select)
	{
		...
		CPrintToChat(param1, "{blue}[SurfTimer-Discord] {green}Type your message. Type 'cancel' to abort.");
	}
...
public Action SayHook(int client, const char[] command, int args)
{
	...
	if(StrEqual(szText, "cancel"))
	{
		...
		CPrintToChat(client, "{blue}[SurfTimer-Discord] {green}Cancelled.");
		return Plugin_Handled;
	}
...
public void SendBugReport(int iClient, char[] szText)
{
	...
	CPrintToChat(iClient, "{blue}[SurfTimer-Discord] {green}Bug report sent - thanks.");
}
...
public void SendCallAdmin(int iClient, char[] szText)
{
	...
	CPrintToChat(iClient, "{blue}[SurfTimer-Discord] {green}An admin was called - thanks.");
}
