		...
		case MenuAction_Select:
		{
			...
			if(StrEqual(sInfo, "-1") || StrEqual(sInfo, "-2")){
				...
				if(g_PlayerData[iClient].iCacheTeam == 3) // CT
				{
					...
					PrintHintText(iClient, "<span><img src='file://{images_econ}/econ/characters/customplayer_ct_map_based.png'></span>");
				...
				}else if(g_PlayerData[iClient].iCacheTeam == 2) // T
				{
					...
					PrintHintText(iClient, "<span><img src='file://{images_econ}/econ/characters/customplayer_t_map_based.png'></span>");
...
int MenuAgentSelect_Callback(Menu hMenu, MenuAction mAction, int iClient, int iSlot)
{
	switch(mAction)
	{
		case MenuAction_Select:
		{
			...
			if(CS_TEAM_CT == GetAgentModelTeam(StringToInt(sInfo))) // CT
			{
				g_PlayerData[iClient].iAgentCT = StringToInt(sInfo);
			}else if(CS_TEAM_T == GetAgentModelTeam(StringToInt(sInfo))) // T
			{
				g_PlayerData[iClient].iAgentT = StringToInt(sInfo);
			}
