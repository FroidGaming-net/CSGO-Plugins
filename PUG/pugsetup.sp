...
public Plugin myinfo = {
    name = "CS:GO PugSetup (Modified Version)",
...
public void OnPluginStart() {
  ...
  g_MapVoteTimeCvar =
      CreateConVar("sm_pugsetup_mapvote_time", "60",
...
public void OnClientDisconnect_Post(int client) {
  ...
  if (g_GameState == GameState_Live) {
    if (numPlayers <= 3) {
      PugSetup_MessageToAll("Match cancelled because less than 3 players.");
      CreateTimer(20.0, Timer_EndMatch2);
    } else if (numPlayers >= 4 && numPlayers <= 5) {
      PugSetup_MessageToAll("Match will be cancelled if less than 3 players.");
    }
  }
}
...
public Action Timer_CheckReady(Handle timer) {
  ...
  if (totalPlayers >= PugSetup_GetPugMaxPlayers()) {
    CreateTimer(30.0, Timer_DelaySpawn, _, TIMER_FLAG_NO_MAPCHANGE);
  }
...
public Action Timer_DelaySpawn(Handle timer)
{
...
public void StatusHint(int readyPlayers, int totalPlayers) {
  ...
  if (captainsNeeded) {
    ...
    if (g_OnDecidedMap) {
      PrintHintTextToAll("<font color='#32CD32'>%i</font>/<font color='#32CD32'>10</font> players for start the match", totalPlayers);
    }else{
      PrintHintTextToAll("<font color='#32CD32'>%i</font>/<font color='#32CD32'>10</font> players for start map voting", totalPlayers);
    }
  }
}
...
public Action Timer_EndMatch2(Handle timer) {
  if (g_GameState == GameState_Live) {
    int numPlayers = 0;
    for (int i = 1; i <= MaxClients; i++) {
      if (IsPlayer(i)) {
...
public Action Event_RoundEnd(Event event, const char[] name, bool dontBroadcast) {
    ...
    if (g_DoVoteForKnifeRoundDecisionCvar.IntValue != 0) {
      ...
      for(int x = 1; x <= MaxClients; x++)
      {
        if(IsClientInGame(x) && !IsFakeClient(x)){
          if(GetClientTeam(x) == g_KnifeWinner && g_KnifeWinner == CS_TEAM_CT)
          {
            Menu hMenu = new Menu(TeamVote_Callback, MenuAction_Select);
            hMenu.SetTitle("Vote Team");
            hMenu.AddItem("stay", "Stay (CT)");
            hMenu.AddItem("switch", "Switch (T)");
            hMenu.ExitBackButton = false;
            hMenu.Display(x, 25);
          }else if(GetClientTeam(x) == g_KnifeWinner && g_KnifeWinner == CS_TEAM_T){
            Menu hMenu = new Menu(TeamVote_Callback, MenuAction_Select);
            hMenu.SetTitle("Vote Team");
            hMenu.AddItem("stay", "Stay (T)");
            hMenu.AddItem("switch", "Switch (CT)");
            hMenu.ExitBackButton = false;
            hMenu.Display(x, 25);
          }
        }
  ...
  if (PugSetup_GetGameState() == GameState_Live) {
    ...
    if ((tScore + ctScore) > 0) {
        if (tScore > ctScore) {
            PugSetup_MessageToAll("\x0B%s \x07%i\x01 - \x04%i\x01 \x09%s", ctName, ctScore, tScore, tName);
        } else if (ctScore == tScore) {
            PugSetup_MessageToAll("\x0B%s \x10%i\x01 - \x10%i\x01 \x09%s", ctName, ctScore, tScore, tName);
        } else {
            PugSetup_MessageToAll("\x0B%s \x04%i\x01 - \x07%i\x01 \x09%s", ctName, ctScore, tScore, tName);
        }
    }
  }
}

public int TeamVote_Callback(Menu hMenu, MenuAction mAction, int iClient, int iSlot)
{
    switch(mAction)
    {
      case MenuAction_Select:
      {
        ...
        if (StrEqual(sInfo, "stay")){
          FakeClientCommand(iClient, "sm_stay");
        }else if (StrEqual(sInfo, "switch")){
          FakeClientCommand(iClient, "sm_swap");
        }
      }
      case MenuAction_End:
      {
