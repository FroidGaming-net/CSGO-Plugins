...
void BuildAvailableWeaponsMenu(int client, bool primary)
{
    ...
    for (int i = 0; i < weapons.Length; i++)
    {
        ...
        if (StrEqual(currentWeapon, weapon))
            if (StrContains(weaponMenuName, "awp", false) > -1) {
                if (CheckCommandAccess(client, "sm_froidapp_premium_plus", ADMFLAG_CUSTOM6)) {
                    menu.AddItem(weapon, weaponMenuName);
                } else {
                    menu.AddItem(weapon, "AWP (Premium Plus Only)", ITEMDRAW_DISABLED);
                }
            } else if (StrContains(weaponMenuName, "five", false) > -1) {
                if (CheckCommandAccess(client, "sm_froidapp_premium_plus", ADMFLAG_CUSTOM6)) {
                    menu.AddItem(weapon, weaponMenuName);
                } else {
                    menu.AddItem(weapon, "Five-SeveN (Premium Plus Only)", ITEMDRAW_DISABLED);
                }
            } else if (StrContains(weaponMenuName, "cz75", false) > -1) {
                if (CheckCommandAccess(client, "sm_froidapp_premium_plus", ADMFLAG_CUSTOM6)) {
                    menu.AddItem(weapon, weaponMenuName);
                } else {
                    menu.AddItem(weapon, "CZ75-A (Premium Plus Only)", ITEMDRAW_DISABLED);
                }
            } else if (StrContains(weaponMenuName, "tec", false) > -1) {
                if (CheckCommandAccess(client, "sm_froidapp_premium_plus", ADMFLAG_CUSTOM6)) {
                    menu.AddItem(weapon, weaponMenuName);
                } else {
                    menu.AddItem(weapon, "Tec-9 (Premium Plus Only)", ITEMDRAW_DISABLED);
                }
            } else {
                menu.AddItem(weapon, weaponMenuName);
            }
